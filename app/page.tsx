...
type ExifTemplate = 'basic' | 'full' | 'gear';
...
function applyExportPreset(preset: ExportPreset): { exportFormat: ExportFormat; quality: number; resolutionMode: ResolutionMode; stripMetadata: boolean } {
  if (preset === 'ultra') {
    return { exportFormat: 'jpeg', quality: 0.98, resolutionMode: 'original', stripMetadata: true };
  }
  if (preset === 'png') {
...
export default function HomePage() {
  ...
  const [exifTemplate, setExifTemplate] = useState<ExifTemplate>('basic');
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-xl flex-col bg-zinc-50 px-4 pt-6 pb-28">
      <header className="mb-4 text-center">
        <h1 className="text-3xl font-semibold tracking-tight text-zinc-900">Aperture Frame</h1>
        <p className="mt-1 text-sm text-zinc-500">Museum-clean borders and reliable iPhone exports.</p>
      </header>
      ...
      <section className="space-y-2">
        ...
        <details className="control-card p-4">
          <summary className="cursor-pointer text-base font-semibold">EXIF line</summary>
          <div className="mt-3 space-y-3">
            <Toggle label="Enable EXIF line" checked={exifEnabled} onToggle={() => setExifEnabled((v) => !v)} />
            {exifEnabled && (
              <>
          ...
          <div className="mt-3 space-y-3">
            <div className="grid grid-cols-1 gap-2">
              ...
              <Choice
                ...
                description="JPEG 0.98 • Original with auto fallback"
              />
              <Choice
      ...
      <section className="fixed inset-x-0 bottom-0 border-t border-zinc-200 bg-white/95 p-3 backdrop-blur">
        <div className="mx-auto flex w-full max-w-xl flex-col gap-2">
          <div className="text-xs text-zinc-500">Export preset: {exportPreset === 'instagram' ? 'Instagram Crisp' : exportPreset === 'ultra' ? 'Ultra JPEG (Fuji-safe)' : 'Lossless PNG'}</div>
          <div className="grid grid-cols-2 gap-2">
